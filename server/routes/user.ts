...
router.post('/register', async (req, res) => {
  try {
    ...
    const rows = await pool.query(query, [name, email, password, phone])
    res.status(200).json({
...
router.patch('/update', async (req, res) => {
  try {
    ...
    const getExistingUser = `SELECT * FROM user WHERE id=?`
    const row = await pool.query(getExistingUser, [id])
